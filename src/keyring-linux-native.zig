//! Linux Secret Service backend: native pure-Zig implementation.
//!
//! Used when the build option `linux-backend == native`. Talks to
//! `org.freedesktop.secrets` directly via the in-tree D-Bus client; no
//! linkage against libsecret or glib.
